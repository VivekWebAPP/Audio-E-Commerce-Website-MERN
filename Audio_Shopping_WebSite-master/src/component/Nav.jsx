...
import logo from '../assets/New Products/Logo/audioace-high-resolution-logo-transparent.png';
import drawerLogo from '../assets/drawer.svg'
import cartLogo from '../assets/logo/card.svg'
import profileLogo from '../assets/profile/profileIcon.png';
...
const Nav = () => {
  ...
  return (
    <nav className='boxRow space_between'>
      ...
      <Link to={'/Login'} state={{ pageName: 'LOGIN' }} className='nav_cardIcon right' style={window.location.hash === '#/Product/:name' ? { animation: 'rotate_Animation .2s linear 2 alternate' } : { animation: 'none' }}>
        <img src={profileLogo} width="25" height="25" alt={'Login'} />
      </Link>
      {
        drawer
      }
