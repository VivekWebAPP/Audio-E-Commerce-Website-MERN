  ...
  return (
    nav_list.map((item, key) => (
      animation ?
        ...
        :
        item.id !== 5 ? <motion.li key={item.id}>
          <Link animate={{ scale: 1.2 }} to={item.navigate} state={{ pageName: item.name }} preventScrollReset={false} className='nav_a'>
            {item.name}
          </Link>
        </motion.li> : <motion.li key={item.id}>
          <Link style={{display:'none'}} animate={{ scale: 1.2 }} to={item.navigate} state={{ pageName: item.name }} preventScrollReset={false} className='nav_a'>
            {item.name}
          </Link>
        </motion.li>
    ))
